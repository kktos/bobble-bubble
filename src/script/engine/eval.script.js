...
// export function interpolateString({gc, vars}, text) {
// 	if(typeof text !== "string") {
// 		return text;
// 	}
// 	return text.replace(/%(.+?)%/, (m, varname) => evalExpr({vars}, varname));
// }
...
export function evalExpr({vars}, varOrExpr) {
	if(Array.isArray(varOrExpr) || typeof varOrExpr === "number")
		return varOrExpr;

	if(typeof varOrExpr === "string") {
		if(varOrExpr.match(/^\$/))
			return evalVar({vars}, varOrExpr.substring(1));
...
export function evalNumber({gc, vars}, nameOrNumber) {
