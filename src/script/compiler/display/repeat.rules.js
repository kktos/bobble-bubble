...
export function repeatRules(parser) {
	...
	$.RULE("layoutRepeat", (options, isMenuItem) => {
		...
		let result = { type: OP_TYPES.REPEAT, step: { pos: [] } };

		const parms = $.SUBRULE(parser.layoutRepeatParms, {
		...
		result.items = $.SUBRULE(parser.layoutRepeatItems, {
			ARGS: [options, isMenuItem],
		});
