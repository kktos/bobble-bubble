...
import type { TMenu, TMenuItemGroup } from "../script/compiler/display/layout/menu.rules";
...
import { renderMenu } from "./display/menu.renderer";
import { loadSprite, renderSprite } from "./display/sprite.renderer";
...
type SelectionSprites = { selectionSprites?: { left?: { ss: unknown; sprite: string }; right?: { ss: unknown; sprite: string } } };
...
export class DisplayLayer extends UILayer {
	...
	prepareMenu(gc: GameContext, op: TMenu & SelectionSprites) {
		...
		for (let idx = 0; idx < op.items.length; idx++) {
			...
			switch (item.type) {
				case OP_TYPES.REPEAT:
					this.repeat(item, (menuitem: TRepeatItem) => menuItems.push(menuitem));
					break;
				// case "text":
				default:
					menuItems.push(item);
		...
		const computeBBox = (items: (TMenuItemGroup | TText | TSprite)[], isGroup = false) => {
	...
	repeat(op: TRepeat, callback: (item: TRepeatItem) => void) {
		const processItem = (item: TText | TSprite, idx: number) => {
			switch (item.type) {
				case OP_TYPES.TEXT:
					(item as TText).text = evalExpr({ vars: this.vars }, item.text) as string;
					break;
				case OP_TYPES.SPRITE:
					(item as TSprite).sprite = evalExpr({ vars: this.vars }, (item as TSprite).sprite) as string;
					break;
			}
